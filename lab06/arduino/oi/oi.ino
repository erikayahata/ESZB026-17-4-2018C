...
void enviaDadosPelaSerial(){              // opcional, para debug
  ...
  Serial.print(buffer[5]); Serial.print("."); // byte mais significativo da ddp
  Serial.println(buffer[6]);                  // byte menos significativo da ddp
}
...
void loop(){                       // Le o valor no potenciometro a cada 1 segundo
  ...
  float A = valorPot/204.6;     // A
  int B = int((A-int(A))*100);  // B
  buffer[5] = byte(A);
  buffer[6] = byte(B);
...
void funcaoDadoRecebido(int x){    // funcao chamada ao receber um dado
  ...
  if( dadoRecebido == 0x10 ){     // nova função
      buffer[2] = buffer[0];
      buffer[3] = buffer[1];
      limiteAlarme = buffer[0] | ( buffer[1] << 8 );
  }
}
